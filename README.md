...
- **Reserva do carrinho (10 min)** (`supabase_migration_cart_holds.sql`): cada navegador tem um carrinho (cookie `copa_cart`); `hold_cart` reserva os itens atomicamente, ajustando ao disponível. O prazo conta do primeiro item e não renova a cada alteração; vencido, as figurinhas voltam a ficar disponíveis e a próxima alteração (ou "Fechar pedido", que abre a tela de confirmação) tenta reservar de novo.
- **Reserva do pedido (24h)** (`supabase_migration_order_reservations.sql` + `supabase_migration_cart_holds.sql`): pedido novo reserva os itens até `reserved_until`; disponível na loja = repetidas − pedidos novos no prazo − carrinhos ativos de outras pessoas. Vencido o prazo sem confirmação, o pedido vira "cancelado (expirado)" — a liberação é imediata (as contas olham o prazo) e a troca de status acontece na próxima leitura (`expire_orders`), sem cron. Criação (`place_order`), edição (`update_order_items`) e confirmação usam trava por vendedor para não vender a mesma repetida duas vezes.
